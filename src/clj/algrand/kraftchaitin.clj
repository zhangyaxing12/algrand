...
(defn fract-to-bin-str
  "Given a number in [0,1], returns a human-readable binary string 
...
(defn binary-interval-of
  "Like interval-of, but the numbers in each pairs are displayed in
  human-readable binary format.  (What actually is done is that the
  original floats are converted to a binary string form, and then that
  string is passed to read-string, creating a wholly different number
  that looks like a binary version of the original number.)"
  [bin-s]
  (vec (map (comp read-string fract-to-bin-str)
            (interval-of bin-s))))
